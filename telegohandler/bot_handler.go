...
// UpdateWithBot bundles an update with its source bot.
// Used by handlers that process updates from multiple bots.
type UpdateWithBot struct {
	telego.Update
	Bot *telego.Bot
}
...
type BotHandler struct {
	...
	updatesWithBot <-chan UpdateWithBot
}
...
// NewMultiBotHandler creates new bot handler that can handle updates from multiple bots, updates must be sent to the
// update channel with the bot pointer, otherwise [BotHandler.Start] will return an error when receiving such update
// Note: Currently no options available, they may be added in future
func NewMultiBotHandler(updates <-chan UpdateWithBot, options ...BotHandlerOption) (*BotHandler, error) {
	bh := &BotHandler{
		baseGroup:      &HandlerGroup{},
		updatesWithBot: updates,
	}
...
func (h *BotHandler) Start() error {
	...
	for {
		...
		case <-h.stop:
			...
			return nil
		case update, ok := <-h.updatesWithBot:
			if !ok {
				return nil
			}
